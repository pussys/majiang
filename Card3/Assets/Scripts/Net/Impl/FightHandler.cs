...
public class FightHandler : HandlerBase
{
    public override void OnReceive(int subCode, object value)
    {
        switch (subCode)
        {
            case FightCode.GET_CARD_SRES:
                getCards(value as List<CardDto>);//获取到卡牌的处理
                break;
            case FightCode.TURN_GRAB_BRO://转换抢地主
                turnGrabBro((int)value);
                break;
            case FightCode.GRAB_LANDLORD_BRO://抢地主成功
                grabLandlordBro(value as GrabDto);
                break;
            case FightCode.TURN_DEAL_BRO://转换出牌
                turnDealBro((int)value);
                break;
            case FightCode.DEAL_BRO://同步出牌
                dealBro(value as DealDto);
                break;
            case FightCode.DEAL_SRES://出牌响应
                dealResponse((int)value);
                break;
            case FightCode.OVER_BRO://结束广播
    ...
    private void dealResponse(int result)
    {
        ...
        {
            //玩家出的牌管不上上一个玩家出的牌
            PromptMsg promptMsg = new PromptMsg("请选择合理手牌", Color.red);
        ...
        {
            //玩家出的牌管不上上一个玩家出的牌
            PromptMsg promptMsg = new PromptMsg("玩家必须出牌", Color.red);
            Dispatch(AreaCode.UI, UIEvent.PROMPT_MSG, promptMsg);
            //重新显示出牌按钮
            Dispatch(AreaCode.UI, UIEvent.SHOW_DEAL_BUTTON, true);
        }
